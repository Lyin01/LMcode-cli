...
安全边界不变：这份确认只作用于驱动自身的放行策略——桌面操作仍逐项走 LMCODE 的常规工具审批，「不受限」不会绕过确认。

验证：agent-core 新增两个方向的单元测试（不受限携带确认 / 标准模式不携带）；真实 Cua Driver 0.28.2 集成测试通过（含不受限模式启动与工具目录加载）。

## 0.9.8

LMD 现在会「销账」。0.9.7 让它记住未完成的事、并在新会话中提醒你，但那份清单此前只能手动删除或在你明确要求时更新——如果之后的会话里事情做完了而没人记得更新，待续记录就会一直挂着。现在，每次会话退出/空闲时的记忆提炼都会带上当前的待续清单（id + 任务，最多最近 12 条）：如果本次会话明确完成了其中某一项（工作已交付、结果达成、你确认完成），它会额外交付一个销账块，把对应记录从待续清单中移除；完成的工作本身仍会照常提炼为任务经验，两者互不混淆。

只会销账、不会误删：只接受提示词里实际提供的 id，模型编造或从别处抄来的 id 一律忽略；删除前逐条复检记录仍是「待续」类型——在列表读取之后被改成偏好/经验的记录不会被碰；销账块与记忆块分开解析，坏的销账块只会被跳过、不影响其它记忆写入。会话内的提醒也从「等你确认完成」升级为「完成即销账」：做完的那一刻顺手更新清单，而不是拖到退出。即使本次没有任何新记忆要写，完成的待续项也会被照常关闭。

边界：销账同样宁缺毋滥——部分完成、只是提到但没做完的都不算；待续清单读取失败时提取照常进行，只是不带销账能力；销账删除与记忆写入一样计入退出收尾，会话关闭会等待它们安全落地。

## 0.9.7

LMD 现在记得「什么还没做完」。此前它知道你做过了什么，现在当一次会话的工作明确没有收尾时——任务做到一半被打断、你说「下次继续」、或已经开始但还没完成——记忆提炼会把它记成一条**待续事项**（写清做到哪了、卡在哪、下一步是什么）。下次打开新会话，它会同「最近的工作」一起被带上：你说一句「继续」，接的就是断在哪里的那条线，而不是从头问一遍。待续事项在设置 → 长期记忆库中有独立的「待续」筛选片与徽章，打开就能看到自己还有哪几件事没做完。

边界与判定刻意收紧：只有任务明显还需要继续时才会记录，话题自然结束、你表示满意时不记；超过 14 天的旧事项自动淡出，每次新会话最多提醒 3 条；确认某项完成后可以让它更新或删除那条记录。提炼在会话退出/空闲与上下文压缩两个入口同时生效——压缩时即使细节被折叠，未完成的工作也会保留为待续记录，而不是被当作「跳过未完成的工作」丢弃。

## 0.9.6

新会话不再从零开始：打开一个新会话发出第一条消息时，LMD 会在后台读取你最近在其它会话中的活动摘要（会话标题、你最后一句输入、项目名与相对时间，最多 5 条），随上下文一起交给模型——你只打一句「继续」，它也能对上你要接着做的事，而不是先反问一轮。摘要只来自本机 `~/.lmcode` 的会话存储，只用于模型理解上下文，不会出现在对话里。

边界上刻意收紧：只有全新会话的首次发言会注入，恢复的会话（自带完整上下文）不注入；子 Agent 不注入；同一会话只注入一次，后续回合不会重复；没有可参考的会话、或读取会话存储失败时静默跳过，绝不打断回合。它与此前的偏好注入各司其职：偏好回答「你是谁」，接续简报回答「你做到了哪」。
