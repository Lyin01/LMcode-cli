...
export function FileActionMenu({ state, onClose }: FileActionMenuProps) {
  ...
  const left = Math.max(8, Math.min(state.x, window.innerWidth - estimateWidth - 8))
  const top = Math.max(8, Math.min(state.y, window.innerHeight - estimateHeight - 8))
