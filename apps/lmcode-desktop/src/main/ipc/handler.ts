...
export function registerAllHandlers(
  ...
  ipcMain.handle('lmcode:openPath', async (_event, input: string): Promise<string> => {
  ...
  ipcMain.handle('lmcode:openExternal', async (_event, url: string): Promise<void> => {
  ...
  ipcMain.handle('lmcode:showItemInFolder', async (_event, input: string): Promise<string> => {
    const target = normalizeOpenPathTarget(input)
  ...
  ipcMain.handle('lmcode:openInVscode', async (_event, input: string): Promise<string> => {
    const target = normalizeOpenPathTarget(input)
    if (target === null) return '仅支持打开绝对路径'
