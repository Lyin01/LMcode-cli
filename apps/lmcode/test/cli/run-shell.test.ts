...
import type { createLmcodeDeviceId as createLmcodeDeviceIdFn } from '@lmcode-cli/config';
...
const mocks = vi.hoisted(() => {
  ...
  class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfigFallback) {
      super('Invalid TUI config in ~/.lmcode/tui.toml; using defaults.');
...
vi.mock('@lmcode-cli/lmcode-sdk', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@lmcode-cli/lmcode-sdk')>();
  return {
    ...actual,
    resolveLmcodeHome: mocks.resolveLmcodeHome,
    LmcodeHarness: class {
      homeDir: string;
      auth = {
      ...
      constructor(...args: unknown[]) {
        ...
        this.homeDir = options?.homeDir ?? '/tmp/lmcode-test-home';
        if (mocks.harnessCreatesDeviceIdOnConstruction) {
          mocks.createLmcodeDeviceId(this.homeDir);
        }
        mocks.lmcodeHarnessConstructor(...args);
      }
    },
  };
});

vi.mock('@lmcode-cli/config', async () => {
  const actual = await vi.importActual<typeof import('@lmcode-cli/config')>(
    '@lmcode-cli/config',
  );
  return {
    ...actual,
...
vi.mock('../../src/tui/index', () => ({
  LmcodeTUI: class {
    ...
    constructor(...args: unknown[]) {
      mocks.lmcodeTuiConstructor(this, ...args);
    }
...
describe('runShell', () => {
  afterEach(() => {
    ...
    mocks.createLmcodeDeviceId.mockImplementation(() => 'device-1');
    mocks.resolveLmcodeHome.mockImplementation(
      (homeDir?: string) => homeDir ?? '/tmp/lmcode-test-home',
    );
    mocks.harnessCreatesDeviceIdOnConstruction = false;
  });

  it('constructs LmcodeHarness and LmcodeTUI with startup input', async () => {
    ...
    expect(mocks.lmcodeHarnessConstructor).toHaveBeenCalledWith(
    ...
    expect(mocks.lmcodeTuiConstructor).toHaveBeenCalledTimes(1);

    const [, harness, startupInput] = mocks.lmcodeTuiConstructor.mock.calls[0]!;
  ...
  it('detects auto theme and forwards config parse warnings as startup notice', async () => {
    ...
    const [, , startupInput] = mocks.lmcodeTuiConstructor.mock.calls[0]!;
    expect(startupInput).toMatchObject({
      startupNotice: 'Invalid TUI config in ~/.lmcode/tui.toml; using defaults.',
      resolvedTheme: 'light',
      tuiConfig: {
        theme: 'auto',
        editorCommand: 'vim',
