...
import { LmcodeTUI, type LmcodeTUIStartupInput, type TUIState } from "#/tui/lmcode-tui";
import {
...
function makeStartupInput(
  cliOptions: Partial<LmcodeTUIStartupInput["cliOptions"]> = {},
  tuiConfig: Partial<LmcodeTUIStartupInput["tuiConfig"]> = {},
  resolvedTheme: LmcodeTUIStartupInput["resolvedTheme"] = "dark",
): LmcodeTUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
function makeHarness(session = makeSession(), overrides: Record<string, unknown> = {}) {
  return {
    getConfig: vi.fn(async () => ({
      models: {
        k2: { model: "lmcode-cli-v1", maxContextSize: 100 },
      },
    })),
...
function makeDriver(harness: ReturnType<typeof makeHarness>, input: LmcodeTUIStartupInput) {
  const driver = new LmcodeTUI(harness as never, input) as unknown as StartupDriver;
...
describe("LmcodeTUI startup", () => {
  ...
  it("passes the CLI model override when creating a fresh startup session", async () => {
    ...
    const driver = makeDriver(harness, makeStartupInput({ model: "lmcode/k2.5" }));
  ...
  it("applies the CLI model override when resuming a startup session", async () => {
    ...
    const driver = makeDriver(
      harness,
      makeStartupInput({ continue: true, model: "lmcode/k2.5" }),
    );

    await expect(driver.init()).resolves.toBe(true);

    expect(session.setModel).toHaveBeenCalledWith("lmcode/k2.5");
    expect(driver.state.appState.model).toBe("lmcode/k2.5");
  });
