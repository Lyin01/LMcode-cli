...
beforeEach(async () => {
  ...
  homeDir = await mkdtemp(join(tmpdir(), 'lmcode-cli-log-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'lmcode-cli-log-work-'));
  oldHome = process.env['LMCODE_HOME'];
  process.env['LMCODE_HOME'] = homeDir;
});
...
afterEach(async () => {
  ...
  if (oldHome === undefined) {
    ...
    process.env['LMCODE_HOME'] = oldHome;
  }
