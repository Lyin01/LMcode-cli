/**
 * Real-LLM smoke. Opt-in only — runs when `LMCODE_E2E_REAL=1` is set, so
 * `make test` / CI never touches the network. Goes through the SDK's
 * `LmcodeHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 *   LMCODE_E2E_REAL     — set to "1" to enable this suite
 *   LMCODE_E2E_MODEL    — model alias override (default: config's default)
 *   LMCODE_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   LMCODE_E2E_WORKDIR  — workspace directory (default: /tmp/lmcode-e2e)
 */
...
const ENABLED = process.env['LMCODE_E2E_REAL'] === '1';
type TurnEndedEvent = Extract<Event, { readonly type: 'turn.ended' }>;

describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through LmcodeHarness',
    async () => {
      const workDir = process.env['LMCODE_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['LMCODE_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['LMCODE_E2E_MODEL'];
