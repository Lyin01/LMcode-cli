...
import {
  ...
  LMCODE_PLUGIN_MARKETPLACE_URL_ENV,
} from '#/constant/app';
...
export async function loadPluginMarketplace(
  ...
  const location = resolveMarketplaceLocation(
    options.source ?? process.env[LMCODE_PLUGIN_MARKETPLACE_URL_ENV] ?? LMCODE_PLUGIN_MARKETPLACE_URL,
    options.workDir,
  );
...
function resolveMarketplaceLocation(source: string, workDir: string): MarketplaceLocation {
  ...
  if (trimmed.length === 0) {
    throw new Error(`${LMCODE_PLUGIN_MARKETPLACE_URL_ENV} 不能为空.`);
  }
