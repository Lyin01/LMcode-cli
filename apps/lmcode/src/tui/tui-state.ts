...
import { createLmcodeTUIThemeBundle, type LmcodeTUIThemeBundle } from './theme/bundle';
...
export function createTUIState(options: LmcodeTUIOptions): TUIState {
  const initialAppState = options.initialAppState;
  const theme = createLmcodeTUIThemeBundle(initialAppState.theme, options.resolvedTheme);
