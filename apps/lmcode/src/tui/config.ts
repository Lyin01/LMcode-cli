 ...
 * terminal UI preferences for the lmcode client.
 */
...
export const INVALID_TUI_CONFIG_MESSAGE =
  '~/.lmcode/tui.toml 中的 TUI 配置无效；使用默认配置。';
...
export function renderTuiConfig(config: TuiConfig): string {
  return `# ~/.lmcode/tui.toml
# Terminal UI preferences for lmcode.
# Agent/runtime settings stay in ~/.lmcode/config.toml.
