...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `LmcodeTUI` calls `setPlanInfo` with
  ...
  // when LmcodeTUI routes a `subagent.event` with this tool call
   ...
   * this: plan-file mode leaves `args.plan` empty, so `LmcodeTUI` fetches
  ...
  // ── Subagent API (called by LmcodeTUI event routing) ───────────────
