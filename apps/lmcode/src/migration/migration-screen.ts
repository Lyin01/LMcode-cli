 ...
 * conventions in `apps/lmcode/src/tui/components/dialogs/choice-picker.ts`.
 *
...
} from '@lmcode-cli/migration-legacy';
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  private renderResult(width: number): string[] {
    ...
    if (r !== undefined) {
      ...
      // safely be held by two installs at once). lmcode's normal auth flow
