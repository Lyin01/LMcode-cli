...
| `lm --yolo` | 显式使用 yolo 权限模式，绕过敏感路径与工作区外写入确认 |
| `/config` | 配置模型服务 |
| `/model` | 切换模型和思考强度 |
| `/permission` | 切换权限模式 |
| `/plan` | 开关计划模式 |
...
- 文件边界检查覆盖声明了文件访问的内置文件工具，以及 Bash（计划模式和只读沙箱把 Bash 视为未声明路径的写操作）。MCP 工具和用户自定义工具仍不声明文件访问——需要约束它们时请使用 `manual` 或配置 deny 规则。

当前版本启动时默认是 `auto`：普通工具自动批准，敏感文件、Git 控制路径和当前目录外写入仍会询问。需要完全自动批准时使用 `lm --yolo` 或 `/yes`。希望每一步都确认时，进入 TUI 后用 `/permission` 切到 `manual`。
