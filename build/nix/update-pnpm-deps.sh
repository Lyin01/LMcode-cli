...
CACHE_VERSION="v1"
FETCHER_VERSION="3"
CACHE_FILE="$ROOT/.git/lmcode/pnpm-deps-hashes-$CACHE_VERSION.json"
RESTORE_ORIG_HASH=0
...
if nix build --no-link '.#lmcode-pnpm-deps' >/dev/null 2>&1; then
...
BUILD_OUT="$(nix build --no-link --print-build-logs '.#lmcode-pnpm-deps' 2>&1 || true)"
...
if ! nix build --no-link '.#lmcode-pnpm-deps'; then
