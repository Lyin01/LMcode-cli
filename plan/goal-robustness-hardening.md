...
| 5 | 测试确定性 / env-fragility（承接修 main 红发现的问题） | 测试健壮性 | ✅ 已修 458cba1 的 session_context 快照（日期/cwd 归一化，见修 main 红那批提交）；全仓扫描其余：字面日期均为固定 fixture 或 `vi.setSystemTime` mock、机器路径均为固定测试输入——**无其余潜在 env-fragile 测试** |
| 4 | 关键错误分支的测试覆盖 | 错误路径覆盖 | 🔵 进行中——**主交付模式**。✅ 已补 compaction 熔断器测试 `49afab8`（非阻塞多步压缩连续失败→断言恰好 3 次尝试 + `compaction_circuit_open`；5 次运行确定性通过）。下批候选：retry `retryAfterMsFor` 的 NaN/Infinity/非数值守卫、oauth/store 错误路径、hooks 超时/失败路径 |
...
- **2026-07-07 · 完成 #4 首个目标**：compaction 熔断器测试落地（`49afab8`，41/41 通过、5 次运行确定）。关键 harness 手法：自定义 `generate` 按系统提示词区分压缩调用（`'You are a conversation context compaction assistant'`）令其抛非可重试 400，主循环用唯一 id 的 missing-tool 调用驱动多步；`maxCompactionPerTurn` 设高（20）以隔离熔断器与每回合上限。下批 #4 候选见状态表。
- **2026-07-07 · #4 retryAfterMs 边界 + #3 hooks 判定**：补 `retryAfterMsFor` 的 NaN/Infinity/非数值/零 守卫测试（`54f34dd`，14/14）。#3 hooks 空 catch 判定为有意。**观察**：spot-check 的错误恢复路径大多**已有覆盖**（blobref EEXIST 去重、per-id-json-store 的损坏 JSON/非法 basename/isValid 跳过均已测）。**本仓错误处理既健壮又覆盖良好，健壮性目标已接近收益递减**。
- **2026-07-07 · 修 main pre-existing 红（插曲）**：跑全量发现 main 被 458cba1 带红（env-fragile 测试）。root-cause 并根治 5 个失败（session-cancel 上下文撑爆、session-prompt-events 位置断言、config×2+subagent-host 的日期/cwd 快照未归一化）。全量转绿 4857/0，进 PR #9。
- **2026-07-07 · 完成 #3 + #5，健壮性目标收尾**：#3 剩余空 catch（self-healing/rpc/sinks）全判为有意。#5 测试确定性扫描干净（除已修的 458cba1 那批）。**结论：本仓运行时错误处理工程优秀、覆盖良好；真实 bug 稀少，价值主要来自已交付的关键覆盖（熔断器、retryAfterMs 边界）+ 修好 main 红。健壮性目标实质完成。**
