...
6. [TUI File Layout (apps/lmcode)](#tui-file-layout-apps-lmcode)
7. [Module Responsibilities (apps/lmcode)](#module-responsibilities-apps-lmcode)
8. [LmcodeTUI Internal Sections (apps/lmcode)](#lmcodetui-internal-sections-apps-lmcode)
9. [Where New Features Go (apps/lmcode)](#where-new-features-go-apps-lmcode)
10. [TUI Coding Conventions (apps/lmcode)](#tui-coding-conventions-apps-lmcode)
11. [How to Set Themes (apps/lmcode)](#how-to-set-themes-apps-lmcode)
12. [MCP (apps/lmcode)](#mcp-apps-lmcode)
13. [Slash Commands (apps/lmcode)](#slash-commands-apps-lmcode)
...
| `node-sdk` | `packages/node-sdk/` | Node.js SDK (`LmcodeHarness`, `Session`) consumed by the app |
...
| `apps/lmcode` | `apps/lmcode/` | CLI and terminal UI application (`lm` command) |

### Terminology

- When the user says **"agent"** or **"session"**, they mean the `packages/agent-core` runtime (`Session`, `Agent`, turn loop), not the assistant.
- **"app"** / **"TUI"** / **"CLI"** refers to `apps/lmcode`.
- **"SDK"** refers to `@lmcode-cli/lmcode-sdk` exported from `packages/node-sdk`.
- **"LLM layer"** refers to `packages/ltod`.
- **"memory"** refers to `packages/memory` task-experience records.

### Cross-package Import Rules

- `apps/lmcode` must use core capabilities **only through `@lmcode-cli/lmcode-sdk`**. Never import `@lmcode-cli/agent-core` directly in app code.
- `packages/agent-core` must not depend on `apps/lmcode`.
...
- Existing `console.error` in `apps/lmcode/src/tui/tui-state.ts` should be treated as a legacy escape hatch, not a pattern to copy.
...
## TUI File Layout (apps/lmcode)

`apps/lmcode` is the terminal UI / CLI app. The entry chain is:
...
- `src/tui/lmcode-tui.ts`: the TUI master assembler, responsible for wiring state, layout, editor, session, SDK events, and dialogs together.
...
## Module Responsibilities (apps/lmcode)
...
- `LmcodeTUI` coordinates; it does not accumulate complex business rules. New logic that can be tested independently should be split into `commands`, `components`, `reverse-rpc`, or `utils` first.
- `commands` only owns slash-command declaration, parsing, and the parsed-result types. The actual execution can be dispatched from `LmcodeTUI`, but complex logic should continue to sink downward.
...
- Resume replay orchestration lives in the `Session Replay` section of `LmcodeTUI`, because it intentionally drives the same stateful render hooks as live events. Stateless replay parsing, limiting, and projection helpers belong in `src/tui/utils/message-replay.ts`.
- `apps/lmcode` may only use core capabilities through `@lmcode-cli/lmcode-sdk`. Do not import `@lmcode-cli/agent-core` directly in app code.

---

## LmcodeTUI Internal Sections (apps/lmcode)

`src/tui/lmcode-tui.ts` is large. When you modify it, place code into the existing responsibility section — do not just drop it where it happens to be convenient.

- Types and state creation: `LmcodeTUIStartupInput`, `TUIState`, `createInitialAppState`, `createTUIState`. Before adding new global UI state, decide whether it really belongs in `TUIState`.
...
If a section keeps growing, split pure functions, state projections, presentation components, and handler logic into the corresponding directories rather than continuing to expand `LmcodeTUI`.

---

## Where New Features Go (apps/lmcode)
...
- New CLI subcommands: put them under `src/cli/sub/`, with non-interactive command logic only; when SDK access is needed, go through `@lmcode-cli/lmcode-sdk`.
- New slash commands: first change definition, parsing, and types under `src/tui/commands/`; put the execution entry into the slash-command handler section of `LmcodeTUI`; split complex execution logic into `utils` or focused components when it has no reason to stay in `LmcodeTUI`.
...
## TUI Coding Conventions (apps/lmcode)

- Do not over-encapsulate, especially for one- or two-line functions — do not introduce a two-layer wrapper, just inline.
- Functions with no state / UI side effects do not belong as private methods on the `LmcodeTUI` class; put them in external utils.
...
- `bundle.ts` packs `colors`, `styles`, and `markdownTheme` into a `LmcodeTUIThemeBundle`.
...
- The real apply step goes through `LmcodeTUI.applyTheme`, which should update `state.theme`, `state.appState.theme`, and notify the relevant components to refresh their palette.
...
/mcp panel → write mcp.json → McpConnectionManager → StdioClient/HttpClient
                 ↑                                          ↓
           ~/.lmcode/mcp.json                   MCP server process
                                                      (launched via npx)
```

- **Config**: `~/.lmcode/mcp.json` (user-global) and `<cwd>/.lmcode/mcp.json` (project-local). Project entries override user entries with the same key.
...
- **TUI panel**: `apps/lmcode/src/tui/commands/mcp.ts` — `/mcp` slash command with custom `McpPickerComponent`.
...
Edit the `RECOMMENDED` array in `apps/lmcode/src/tui/commands/mcp.ts`.
...
## Slash Commands (apps/lmcode)

All slash commands are declared in `src/tui/commands/registry.ts` and dispatched in `src/tui/commands/dispatch.ts`. Beyond the session-config-modelling helpers documented in `LmcodeTUI`, these commands carry non-trivial state or backend integration:
...
- **Version source**: `src/cli/update/cdn.ts` — fetches `api.github.com/repos/Lyin01/LMcode-cli/releases/latest`, strips `v` prefix from `tag_name`.
- **Cache**: `src/cli/update/cache.ts` — reads/writes `~/.lmcode/updates/latest.json`.
- **Compare**: `src/cli/update/select.ts` — `semver.gt(latest, current)`.
- **TUI startup**: `checkForUpdates()` in `lmcode-tui.ts` calls `refreshUpdateCache()` then `readUpdateCache()` + `selectUpdateTarget()`.
...
- **Constant**: `src/constant/app.ts` — `LMCODE_CDN_LATEST_URL`, `LMCODE_GITHUB_REPO`.
...
- **Extraction triggers**:
  ...
  - Idle timer: after 10 minutes of no user input, `LmcodeTUI.performIdleMemoryExtraction()` calls `session.extractMemoriesOnExit()`. Cooldown: 10 minutes. Compaction extraction updates the cooldown timestamp to avoid duplicates.
...
- **Manual injection**: users can still browse and inject existing memos via the `/memory` TUI picker (`apps/lmcode/src/tui/managers/dialog-manager.ts`).
...
- **Tracker**: `packages/memory/src/dream.ts` — `DreamTracker`, persisted to
  `<lmcodeHomeDir>/dream-lock.json` (default `~/.lmcode/dream-lock.json`).
- **Store**: `packages/memory/src/store.ts` — `MemoryMemoStore`, persisted to
  `<lmcodeHomeDir>/memory/entries.jsonl`.
