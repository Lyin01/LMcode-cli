...
const { LmcodeHarness, Session } = await import('#/index');
...
describe('Session skills', () => {
  it('lists session skills without exposing content', async () => {
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.lmcode/skills/review/SKILL.md')).toBe(true);
  ...
  it('resolves user skills from the OS home directory, independently of LMCODE_HOME', async () => {
    ...
    const harness = new LmcodeHarness({ identity: TEST_IDENTITY });
