...
import { LmcodeError, LmcodeHarness } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'lmcode-sdk-config-'));
  tempDirs.push(dir);
  return dir;
}

const COMPLETE_TOML = `
default_model = "lmcode-for-coding"
...
[providers.lmcode-for-coding.env]
GOOGLE_CLOUD_PROJECT = "project-1"

[models.lmcode-for-coding]
provider = "lmcode-for-coding"
...
describe('SDK config TOML', () => {
  it('parses the documented config shape and keeps TUI-only fields in raw', () => {
    ...
    expect(config.defaultModel).toBe('lmcode-for-coding');
    ...
    expect(config.models?.['lmcode-for-coding']).toMatchObject({
      provider: 'lmcode-for-coding',
      model: 'lmcode-for-coding',
  ...
  it('writes typed fields in snake_case and preserves unknown raw sections', async () => {
    ...
    await writeConfigFile(configPath, {
      ...config,
      defaultModel: 'lmcode-for-coding',
      loopControl: {
        ...config.loopControl,
        maxStepsPerTurn: 42,
      },
    ...
    expect(text).toContain('default_model = "lmcode-for-coding"');
...
describe('LmcodeHarness config API', () => {
  ...
  it('does not write invalid config patches', async () => {
    ...
    await expect(setInvalidConfig).rejects.toBeInstanceOf(LmcodeError);
