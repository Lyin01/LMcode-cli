...
const { LmcodeHarness } = await import('#/index');
...
describe('Session.prompt events', () => {
  ...
  it('emits mapped turn events through Session.onEvent', async () => {
    ...
    try {
      ...
      expect(fakeProviderState.calls[0]?.systemPrompt).toContain('当前可用的技能列表');
...
async function configureFakeProvider(harness: InstanceType<typeof LmcodeHarness>): Promise<void> {
