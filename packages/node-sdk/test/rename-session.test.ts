...
import { LmcodeError, LmcodeHarness, type Event } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'lmcode-sdk-rename-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('LmcodeHarness.renameSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    try {
      ...
      await expect(missingRename).rejects.toBeInstanceOf(LmcodeError);
