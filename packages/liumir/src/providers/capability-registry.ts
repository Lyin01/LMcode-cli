...
/** GLM-5.3-Flash accepts image input natively even when served via an
 *  OpenAI-compatible gateway. Without this entry it resolves to
 *  UNKNOWN_CAPABILITY and the visual-fallback MCP gets injected needlessly. */
const GLM_VISION_TOOL_CAPABILITY: ModelCapability = Object.freeze({
  image_in: true,
  video_in: false,
  audio_in: false,
  thinking: false,
...
const GLM_VISION_PREFIXES = ['glm-5.3-flash'] as const;
...
const OPENAI_LEGACY_CAPABILITY_CATALOG: readonly CapabilityCatalogEntry[] = [
  ...
  {
    matches: (name) => hasPrefix(name, GLM_VISION_PREFIXES),
    capability: GLM_VISION_TOOL_CAPABILITY,
  },
];
