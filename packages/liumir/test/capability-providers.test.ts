...
describe('LmcodeChatProvider.getCapability', () => {
  function make(model: string): LmcodeChatProvider {
  ...
  it('does not infer capabilities from LMcode model names', () => {
    for (const model of [
      'lmcode-for-coding',
      'lmcode',
      'lmcode-k2-turbo-preview',
      'lmcode-k2.5',
      'lmcode-thinking-preview',
    ]) {
  ...
  it('explicit model arg overrides this.modelName', () => {
    const provider = make('lmcode-k2-turbo-preview');
...
describe('OpenAILegacyChatProvider.getCapability', () => {
  ...
  it('glm-5.3-flash → image_in + tool_use (vision-capable GLM tier)', () => {
  ...
  it('glm-5.3-flash name casing is normalized', () => {
    expect(make('GLM-5.3-Flash').getCapability().image_in).toBe(true);
  });

  it('text-only GLM tiers stay UNKNOWN_CAPABILITY (no blanket glm- prefix)', () => {
    // Only the 5.3-flash tier is catalogued as vision-capable; text-only
    // tiers must not silently gain image_in, or the visual-fallback MCP
    // suppression logic would misfire for them.
    expect(make('glm-4-flash').getCapability()).toEqual(UNKNOWN_CAPABILITY);
  });
