...
把旧版 scream-cli 的数据（`~/.scream/` 时代的会话、配置）迁移到 LMcode（`~/.lmcode/`）的一次性工具。由 `apps/lmcode/src/migration/` 在启动时检测并触发。
...
- **从 scream-cli 或 LMcode < 0.8 升级**：请先升级到任意 0.8.x – 0.x 版本完成数据迁移，再升级到 1.0+。
