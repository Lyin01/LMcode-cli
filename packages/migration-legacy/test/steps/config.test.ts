...
describe('migrateConfigStep', () => {
  ...
  it('additively merges into a user-modified target config', async () => {
    ...
    expect(cfg).toContain('lmcode/lmcode-for-coding'); // migrated model added
  });
...
`,
    ...
    const r = await migrateConfigStep({ sourceHome: src, targetHome: tgt });
    expect(r.configConflicts).toContain('providers.managed:lmcode');
    const cfg = await readFile(join(tgt, 'config.toml'), 'utf-8');
    expect(cfg).toContain('https://target.example/v1');
    expect(cfg).not.toContain('https://source.example/v1');
  });

  it('drops top-level keys lmcode does not support', async () => {
    await writeFile(
      join(src, 'config.toml'),
  ...
  it('falls back to a sibling file when the target config is unparseable', async () => {
    ...
    expect(
      await readFile(join(tgt, 'config.migrated-from-lmcode-cli.toml'), 'utf-8'),
  ...
  it('a tui.toml-only conflict sets wroteTuiSibling, not wroteSiblingDueToConflict', async () => {
    ...
    expect(
      await readFile(join(tgt, 'tui.migrated-from-lmcode-cli.toml'), 'utf-8'),
    ).toContain('theme');
    // original kept
  ...
  it('drops a kept-provider model missing required schema fields', async () => {
    // `bad-model` references the kept `managed:lmcode` provider but omits
...
`,
  ...
  it('drops a legacy theme outside the lmcode TUI enum', async () => {
  ...
  it('drops a single hook lmcode\'s schema rejects, keeps the rest', async () => {
    await writeFile(
      join(src, 'config.toml'),
      '[[hooks]]\n' +
        'event = "PreToolUse"\n' +
  ...
  it('reports migratedHooks=0 and populates siblingContents when sibling mode kicks in', async () => {
    ...
    // `config.migrated-from-lmcode-cli.toml`. Hooks land in the sibling, NOT in
