...
describe('migrateSkillsStep', () => {
  it('copies SKILL.md bundles and flat .md skills under ~/.lmcode/skills/', async () => {
  ...
  it('returns zero counters when source ~/.lmcode/skills/ is missing', async () => {
    const r = await migrateSkillsStep({ sourceHome: src, targetHome: tgt });
  ...
  it('copies non-skill files at top level too (no filtering)', async () => {
    ...
    // ~/.lmcode/skills/ is preserved verbatim. The new scanner ignores anything
