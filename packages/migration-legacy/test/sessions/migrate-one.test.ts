...
describe('migrateOneSession (tiny-hello-world fixture)', () => {
  ...
  it('reports conflict when an unrelated lmcode session occupies the dir', async () => {
    ...
    // Overwrite state.json with a non-migrated (real) lmcode session.
  ...
  it('re-migrates a target dir left half-written by an interrupted run', async () => {
    ...
    // real lmcode session, so it must be re-migrated, not reported as a
  ...
  it('re-migrates a target dir whose state.json is corrupt', async () => {
    ...
    // is `ses_<uuid>`), not a real lmcode session, so it must be
