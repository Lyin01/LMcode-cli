...
describe('runMigration (end-to-end on multi-workdir fixture)', () => {
  ...
  it('does not copy OAuth credentials into the target', async () => {
    ...
    try {
      ...
      await expect(
        readFile(join(tgt, 'credentials', 'lmcode.json'), 'utf-8'),
      ).rejects.toThrow();
      // The report tells the user to sign in again in lmcode.
      expect(report.notices.oauthLoginsRequiringRelogin).toContain('lmcode.json');
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  });
