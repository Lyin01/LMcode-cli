/**
 * Plan describing the contents detected under the source `~/.lmcode/` directory.
...
export interface WorkDirEntry {
  readonly oldHashDir: string; // absolute path to ~/.lmcode/sessions/<md5>/
 ...
 * `~/.lmcode/migration-report.json` and surfaced in the terminal summary.
 */
...
export interface MigrationSummary {
  ...
  readonly mcp: {
    ...
    /** Source servers dropped because lmcode's MCP schema rejects them. */
...
export interface MigrationNotices {
   ...
   * lmcode instead. Empty when the legacy install had no OAuth login.
   */
