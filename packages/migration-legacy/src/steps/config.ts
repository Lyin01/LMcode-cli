...
// lmcode's tui.toml `theme` enum (mirrors apps/lmcode TuiThemeSchema).
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 1) Providers — keep only those lmcode's schema accepts.
  ...
  // 2) Models — keep only those lmcode's schema accepts, and not those
  ...
  // 4) Build the migrated top-level — only keys lmcode's schema supports.
  ...
  // 4b) Drop any supported top-level key whose VALUE lmcode's config
