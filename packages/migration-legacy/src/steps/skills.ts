...
/**
 * Copy the user's legacy skills tree (~/.lmcode/skills/) into lmcode's
 * default user skills root (~/.lmcode/skills/). Granularity is one
...
export async function migrateSkillsStep(input: SkillsStepInput): Promise<SkillsStepResult> {
  ...
  for (const name of entries) {
    ...
    // in it — touching it earlier would fail when ~/.lmcode/skills is
