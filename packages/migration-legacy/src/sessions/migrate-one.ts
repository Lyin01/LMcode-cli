...
import { OldSessionStateSchema, type OldSessionState } from '../lmcode-cli-schema.js';
...
export async function migrateOneSession(input: MigrateOneInput): Promise<MigrateOneResult> {
  ...
  if (existsSync(targetDir)) {
    ...
    // A real, unrelated lmcode session occupies the path — a true conflict.
    if (cls === 'foreign') {
 ...
 *  - `foreign`:  a real, unrelated lmcode session occupying the path.
