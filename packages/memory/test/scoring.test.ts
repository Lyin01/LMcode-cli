...
describe('computeRelevanceScore', () => {
  ...
  it('ranks a Chinese memo sharing a real term above one sharing only common chars', () => {
    // Query "缓存优化". The relevant memo shares the term "缓存" (a CJK bigram);
    // the distractor shares only incidental single characters (优/化/存), which
    // would outrank the relevant memo under pure character-level Jaccard.
    const relevant = makeMemo({
      id: 'rel',
      userNeed: '缓存策略',
      approach: '',
      whatFailed: '',
    ...
    const relScore = computeRelevanceScore(relevant, '缓存优化');
    const distScore = computeRelevanceScore(distractor, '缓存优化');

    expect(relScore).toBeGreaterThan(distScore);
  });
});
