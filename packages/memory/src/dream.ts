...
export class DreamTracker {
   ...
   * - <lmcodeHomeDir>/.lmcode/dream-lock.json (buggy double directory)
   * - <lmcodeHomeDir>/sessions/<sessionKey>/.lmcode/dream-lock.json (old per-session)
   *
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          candidates.push(join(sessionsDir, entry.name, '.lmcode', LOCK_FILE));
        }
      }
    } catch {
