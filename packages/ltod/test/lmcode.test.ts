...
function createProvider(stream: boolean = false): LmcodeChatProvider {
...
async function captureRequestBody(
  ...
  (provider as any)._client.chat.completions.create = vi
    ...
    .mockImplementation((params: unknown) => {
      ...
      return Promise.resolve(makeChatCompletionResponse('lmcode-k2'));
    });
...
describe('LmcodeChatProvider', () => {
  ...
  describe('generation kwargs', () => {
    ...
    it('passes constructor generation kwargs into the request body', async () => {
      const provider = new LmcodeChatProvider({
        model: 'lmcode-k2-turbo-preview',
        apiKey: 'test-key',
        stream: false,
  ...
  describe('provider properties', () => {
    ...
    it('throws during generation when no constructor or request API key is provided', async () => {
      ...
      const saved = process.env['LMCODE_API_KEY'];
      delete process.env['LMCODE_API_KEY'];
      try {
        const provider = new LmcodeChatProvider({ model: 'test' });
        await expect(provider.generate('', [], [])).rejects.toThrow(/options\.auth\.apiKey/);
      } finally {
        if (saved !== undefined) {
          process.env['LMCODE_API_KEY'] = saved;
        }
      }
    });
    ...
    it('passes request-scoped auth to the client factory', async () => {
      ...
      const client = {
        chat: {
          completions: {
            create: vi.fn().mockResolvedValue(makeChatCompletionResponse('lmcode-k2')),
          },
        },
      };
      const provider = new LmcodeChatProvider({
    ...
    it('withThinking returns a new instance', () => {
      ...
      expect(newProvider).toBeInstanceOf(LmcodeChatProvider);
  ...
  describe('clone client sharing', () => {
    ...
    function getInternalClient(provider: LmcodeChatProvider): unknown {
