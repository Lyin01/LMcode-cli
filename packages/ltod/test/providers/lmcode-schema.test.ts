import { derefJsonSchema, normalizeLmcodeToolSchema } from '#/providers/lmcode-schema';
...
describe('normalizeLmcodeToolSchema', () => {
  it.each([
    {
      name: 'string enum',
    ...
    ({ property, expectedType }) => {
      ...
      const result = normalizeLmcodeToolSchema(schema);
  ...
  it('fails fast for mixed enum types instead of emitting an unsupported Scream type array', () => {
    ...
    expect(() => normalizeLmcodeToolSchema(schema)).toThrow(
  ...
  it('does not default the root schema itself to string', () => {
    expect(normalizeLmcodeToolSchema({})).toEqual({});
  });
