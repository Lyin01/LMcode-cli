 ...
 * adapter when the native API has no matching level. OpenAI-compatible
 * gateways that accept `max` receive it verbatim; official OpenAI maps `max`
 * to its `xhigh` ceiling; LMcode and Gemini cap `xhigh`/`max` at `high`;
 * Anthropic supports `xhigh`/`max` only on selected models and otherwise
 * clamps to `high`.
 */
...
export interface ChatProvider {
  /** Short identifier for the provider backend (e.g. `"lm"`, `"anthropic"`). */
  readonly name: string;
  /** Model name passed to the upstream API (e.g. `"lmcode-cli-v1-auto"`). */
   ...
   * original. See `LmcodeChatProvider._clone()` for the rationale.
   */
