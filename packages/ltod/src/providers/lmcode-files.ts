...
export interface LmcodeFilesOptions {
  apiKey?: string;
  baseUrl?: string;
 ...
 * {@link LmcodeChatProvider.files}.
 */
export class LmcodeFiles {
  ...
  constructor(options: LmcodeFilesOptions) {
    ...
    if (typeof input === 'string') {
      ...
      if (mimeType === undefined || !mimeType.startsWith('video/')) {
        throw new ChatProviderError(
          `LmcodeFiles.uploadVideo: file extension does not indicate a video type: ${filename}`,
        );
      }
    ...
    return resolveAuthBackedClient(
      ...
      (a) => {
        ...
        return new OpenAIClient({
          apiKey: requireProviderApiKey('LmcodeFiles.uploadVideo', a, this._apiKey),
