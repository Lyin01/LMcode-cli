...
import { normalizeLmcodeToolSchema } from './lmcode-schema';
...
import { LmcodeFiles } from './lmcode-files';
import {
...
export interface LmcodeOptions {
...
function convertTool(tool: Tool): OpenAIToolParam {
  ...
  return {
    ...
    function: {
      ...converted.function,
      parameters: normalizeLmcodeToolSchema(tool.parameters),
    },
  };
}
/**
...
class LmcodeStreamedMessage implements StreamedMessage {
...
export class LmcodeChatProvider implements ChatProvider {
  readonly name: string = 'lmcode';
  ...
  private _files: LmcodeFiles | undefined;

  constructor(options: LmcodeOptions) {
    const apiKey = options.apiKey ?? process.env['LMCODE_API_KEY'];
    this._apiKey = apiKey === undefined || apiKey.length === 0 ? undefined : apiKey;
    this._baseUrl = options.baseUrl ?? process.env['LMCODE_BASE_URL'];
   ...
   * messages.
   */
  get files(): LmcodeFiles {
    this._files ??= new LmcodeFiles({
      apiKey: this._apiKey,
      baseUrl: this._baseUrl,
      defaultHeaders: this._defaultHeaders,
  ...
  async generate(
    ...
    try {
      ...
      return new LmcodeStreamedMessage(response, this._stream);
    } catch (error: unknown) {
      throw convertOpenAIError(error);
    }
  }
  ...
  withThinking(effort: ThinkingEffort): LmcodeChatProvider {
  ...
  withGenerationKwargs(kwargs: GenerationKwargs): LmcodeChatProvider {
    return this._withGenerationKwargs(kwargs);
  }

  withMaxCompletionTokens(maxCompletionTokens: number): LmcodeChatProvider {
    return this._withGenerationKwargs({ max_completion_tokens: maxCompletionTokens });
  }

  withExtraBody(extraBody: ExtraBody): LmcodeChatProvider {
  ...
  private _createClient(auth: ProviderRequestAuth | undefined): OpenAI {
    return resolveAuthBackedClient(
      ...
      (a) => {
        ...
        return new OpenAI({
          apiKey: requireProviderApiKey('LmcodeChatProvider', a, this._apiKey),
          baseURL: this._baseUrl,
          defaultHeaders,
        });
      },
  ...
  private _withGenerationKwargs(kwargs: GenerationKwargs): LmcodeChatProvider {
    const clone = this._clone();
    clone._generationKwargs = { ...clone._generationKwargs, ...kwargs };
    return clone;
  }

  private _clone(): LmcodeChatProvider {
    const clone = Object.assign(
      Object.create(Object.getPrototypeOf(this) as object) as LmcodeChatProvider,
      this,
    );
    clone._generationKwargs = { ...this._generationKwargs };
    // Do not share the memoized LmcodeFiles instance with the clone; let it be
    ...
    // construction; instead build a new LmcodeChatProvider when a real new
    // client is required.
    return clone;
  }
}
