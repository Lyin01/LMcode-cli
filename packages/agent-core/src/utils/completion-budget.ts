...
export function resolveCompletionBudget(args: {
  ...
  const fromNew = parseEnvBudget(env['LMCODE_MODEL_MAX_COMPLETION_TOKENS']);
  if (fromNew !== 'absent') {
    return fromNew === 'disabled' ? undefined : { hardCap: fromNew };
  }
  const fromLegacy = parseEnvBudget(env['LMCODE_MODEL_MAX_TOKENS']);
