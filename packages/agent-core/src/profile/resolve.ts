...
function buildTemplateVars(
  ...
  // Default the timestamp to day precision so the system-prompt prefix stays
  // byte-identical across sessions on the same day. A fresh millisecond every
  // session would otherwise make the (large) system prompt a guaranteed
  // prefix-cache miss for providers that cache on an exact prefix (e.g.
  // DeepSeek, whose cache hits cost ~98% less). The prompt uses this only as a
  // coarse reference — exact time is fetched via the Bash tool when needed.
  // Callers may still pin an explicit value via `context.now`.
  const now =
