...
import { parseMemoryMemos } from '@lmcode/memory';
...
export class FullCompaction {
    ...
    this.strategy =
      ...
      new DefaultCompactionStrategy(
        () => agent.config.modelCapabilities.max_context_tokens,
        {
          ...
          reservedContextSize:
            agent.lmcodeConfig?.loopControl?.reservedContextSize ??
            DEFAULT_COMPACTION_CONFIG.reservedContextSize,
          triggerRatio:
            agent.lmcodeConfig?.loopControl?.compactionTriggerRatio ??
  ...
  private beginAutoCompaction(throwOnLimit: boolean = true): boolean {
    ...
    if (this.compactionCountInTurn >= maxCompactions) {
      if (throwOnLimit) {
        throw new LmcodeError(ErrorCodes.CONTEXT_OVERFLOW, `Compaction limit exceeded (${String(maxCompactions)})`, {
  ...
  private async compactionWorker(
    ...
    try {
      ...
      if (!isAbortError(error)) {
        ...
        if (blockedByTurn) {
          if (isLmcodeError(error) && error.code === ErrorCodes.AUTH_LOGIN_REQUIRED) throw error;
          throw new LmcodeError(ErrorCodes.COMPACTION_FAILED, String(error), { cause: error });
        }
      }
    }
  }
