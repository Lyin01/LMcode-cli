...
export class CronManager {
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(process.env['LMCODE_CRON_CLOCK']) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.scheduler = createCronScheduler({
      ...
      isKilled: () => process.env['LMCODE_DISABLE_CRON'] === '1',
      ...
      // P1.8: `LMCODE_CRON_MANUAL_TICK=1` forces the scheduler into
      ...
      pollIntervalMs:
        process.env['LMCODE_CRON_MANUAL_TICK'] === '1'
          ? null
          : opts.pollIntervalMs,
    });
   ...
   * Stale judgment.
   *
   *   - `LMCODE_CRON_NO_STALE=1` short-circuits to false (bench).
  ...
  isStale(task: CronTask): boolean {
    if (process.env['LMCODE_CRON_NO_STALE'] === '1') return false;
   ...
   *
   * Gated on `LMCODE_CRON_MANUAL_TICK=1` for two reasons:
   *
   ...
   * Set `LMCODE_CRON_DEBUG=1` to surface the swallowed error to stderr —
   * mirrors `scheduler.ts`'s debugLog pattern so bench debugging can
   * see a bad tick.
   */
  private bindSigusr1(): void {
    if (process.platform === 'win32') return;
    if (process.env['LMCODE_CRON_MANUAL_TICK'] !== '1') return;
