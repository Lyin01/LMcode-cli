 ...
 *    state under `<LMCODE_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.lmcode`).
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("lmcode (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `lmcode (${options.serverName})`;
  }
