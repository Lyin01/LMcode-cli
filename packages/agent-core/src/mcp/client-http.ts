...
/**
 * Wraps the SDK streamable-HTTP transport as a liumir {@link MCPClient}.
...
export function buildMcpHttpHeaders(
  ...
  if (config.bearerTokenEnvVar !== undefined) {
    ...
    if (token === undefined || token.length === 0) {
      throw new LmcodeError(ErrorCodes.CONFIG_INVALID, `MCP HTTP bearer token env var "${config.bearerTokenEnvVar}" is not set or is empty`);
    }
