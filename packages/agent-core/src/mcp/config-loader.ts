...
import { resolveLmcodeHome } from '#/config/path';
...
export interface McpJsonPaths {
  ...
  /** Parent `.lmcode/mcp.json` paths, ordered from root to shallowest. */
...
export function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): McpJsonPaths {
  ...
  return {
    user: join(resolveLmcodeHome(input.homeDir), 'mcp.json'),
    project: join(cwd, '.lmcode', 'mcp.json'),
    parents: findParentMcpJsonPaths(cwd),
  };
}

/** Walk up from `cwd` collecting `.lmcode/mcp.json` paths (root→shallow). */
function findParentMcpJsonPaths(cwd: string): string[] {
  const paths: string[] = [];
  let dir = dirname(cwd);
  for (let i = 0; i < MAX_PARENT_WALK && dir !== dirname(dir); i++) {
    paths.push(join(dir, '.lmcode', 'mcp.json'));
    dir = dirname(dir);
  }
 ...
 *   1. `~/.lmcode/mcp.json` (lowest priority)
 *   2. Parent `.lmcode/mcp.json` files, root→shallow
 *   3. `<cwd>/.lmcode/mcp.json` (highest project priority)
 *
...
async function readMcpJson(filePath: string): Promise<Record<string, McpServerConfig>> {
  ...
  try {
    ...
    throw new LmcodeError(ErrorCodes.CONFIG_INVALID, `Failed to read ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    throw new LmcodeError(ErrorCodes.CONFIG_INVALID, `Invalid JSON in ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
