...
export class PluginManager {
  ...
  async load(): Promise<void> {
    const file = await readInstalled(this.lmcodeHomeDir);
  ...
  async install(source: string): Promise<PluginRecord> {
    ...
    if (resolved.kind === 'local-path') {
      ...
      normalizedRoot = await copyPluginToManagedRoot(this.lmcodeHomeDir, id, sourceRoot);
      ...
      const tmpDir = await mkdtemp(path.join(tmpdir(), 'lmcode-plugin-zip-'));
      try {
        ...
        id = normalizePluginId(parsed.manifest.name);
        normalizedRoot = await copyPluginToManagedRoot(this.lmcodeHomeDir, id, detectedRoot);
        parsed = await parseManifest(normalizedRoot);
      } finally {
  ...
  private async persist(): Promise<void> {
    ...
    await writeInstalled(this.lmcodeHomeDir, { version: 1, plugins: installed });
  }
...
async function copyPluginToManagedRoot(
  ...
  const managedRoot = path.join(lmcodeHomeDir, 'plugins', 'managed', id);
