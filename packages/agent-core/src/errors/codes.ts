...
export type LmcodeErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes];

export interface LmcodeErrorInfo {
...
export const LMCODE_ERROR_INFO = {
  'config.invalid': {
  ...
  'plugin.load_failed': {
    ...
    retryable: true,
    public: true,
    action: 'Fix the installed.json file under $LMCODE_HOME/plugins/ and run /plugins reload.',
  },
  ...
  'shell.git_bash_not_found': {
    ...
    retryable: false,
    public: true,
    action: 'Install Git for Windows from https://gitforwindows.org/ or set LMCODE_SHELL_PATH to a bash.exe.',
  },
...
} as const satisfies Record<LmcodeErrorCode, LmcodeErrorInfo>;
