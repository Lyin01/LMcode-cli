...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'lmcode.log'),
 ...
 *
 *   import { log } from 'lmcode-sdk';
 *   log.info('lmcode starting', { version });
 *
 ...
 * importing `log` at module load (before `LmcodeHarness` configures the root)
...
export function resolveGlobalLogPath(homeDir: string): string {
  return join(homeDir, 'logs', 'lmcode.log');
}
