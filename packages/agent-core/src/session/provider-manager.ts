...
export class ProviderManager implements ModelProvider {
  ...
  resolveAuth(
    ...
    const loginRequired = (cause?: unknown): LmcodeError =>
      new LmcodeError(
    ...
    const fetchAuth = async (force: boolean): Promise<ProviderRequestAuth> => {
      ...
      try {
        ...
        if (!isLmcodeError(error) || error.code !== ErrorCodes.AUTH_LOGIN_REQUIRED) {
  ...
  switch (provider.type) {
    ...
    case 'lmcode':
      return {
        type: 'lmcode',
        model,
        baseUrl: providerValue(provider.baseUrl, provider.env, 'LMCODE_BASE_URL'),
        apiKey: providerApiKey(provider),
        generationKwargs: { prompt_cache_key: promptCacheKey },
        ...defaultHeadersField({ ...lmcodeRequestHeaders, ...provider.customHeaders }),
      };
    case 'google-genai':
      return {
        type: 'google-genai',
...
function providerApiKey(provider: ProviderConfig): string | undefined {
  switch (provider.type) {
    ...
    case 'lmcode':
      return providerValue(provider.apiKey, provider.env, 'LMCODE_API_KEY');
    case 'google-genai':
