...
export class Session {
  ...
  constructor(public readonly options: SessionOptions) {
    ...
    this.mcp = new McpConnectionManager({
      oauthService: new McpOAuthService({ lmcodeHomeDir: options.lmcodeHomeDir }),
      log: this.log,
    });
  ...
  private instantiateAgent(
    ...
    return new Agent({
      ...
      homedir,
      lmcodeHomeDir: this.options.lmcodeHomeDir,
      skills: this.skills,
  ...
  private ensureResumeAgentInstantiated(
    ...
    if (meta === undefined) {
      throw new LmcodeError(ErrorCodes.SESSION_STATE_INVALID, `Session agent "${id}" is missing`);
    }
  ...
  private requireMainAgent(): Agent {
    ...
    if (agent === undefined) {
      throw new LmcodeError(ErrorCodes.AGENT_NOT_FOUND, 'Main agent was not found');
    }
    return agent;
  }
