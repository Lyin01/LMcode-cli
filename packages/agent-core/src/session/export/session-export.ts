...
const SESSION_LOG_REL = 'logs/lmcode.log';
const GLOBAL_LOG_REL = 'logs/global/lmcode.log';
...
export async function exportSessionDirectory(input: {
  ...
  if (sessionFiles.length === 0) {
    throw new LmcodeError(ErrorCodes.SESSION_EXPORT_NOT_FOUND, `Session "${input.summary.id}" has no exportable directory at "${sessionDir}"`, {
