...
export const ENV_MODEL_PROVIDER_KEY = '__lmcode__env__';
export const ENV_MODEL_ALIAS_KEY = '__lmcode__env_model__';

const ALLOWED_TYPES: readonly ProviderType[] = ['lmcode', 'anthropic', 'openai'];
...
function fail(message: string): never {
  throw new LmcodeError(ErrorCodes.CONFIG_INVALID, message);
}
...
function parseProviderType(raw: string | undefined): ProviderType {
  if (raw === undefined) return 'lmcode';
  const normalized = raw.toLowerCase() as ProviderType;
  if (!ALLOWED_TYPES.includes(normalized)) {
    fail(
      `LMCODE_MODEL_PROVIDER_TYPE must be one of ${ALLOWED_TYPES.join(', ')}, got "${raw}".`,
    );
  }
  return normalized;
}
...
// fails fast like the other LMCODE_MODEL_* values, instead of silently keeping
...
/**
 * When `LMCODE_MODEL_NAME` is set, synthesize one provider + one model alias from
 * the `LMCODE_MODEL_*` environment variables and make it the default model.
...
export function applyEnvModelConfig(config: LmcodeConfig, env: Env = process.env): LmcodeConfig {
  ...
  const apiKey = trimmed(env['LMCODE_MODEL_API_KEY']);
  if (apiKey === undefined) {
    fail('LMCODE_MODEL_NAME is set but LMCODE_MODEL_API_KEY is missing.');
  }

  const maxContextRaw = trimmed(env['LMCODE_MODEL_MAX_CONTEXT_SIZE']);
  const maxContextSize =
    maxContextRaw === undefined
      ? DEFAULT_MAX_CONTEXT_SIZE
      : parsePositiveInt(maxContextRaw, 'LMCODE_MODEL_MAX_CONTEXT_SIZE');

  const type = parseProviderType(trimmed(env['LMCODE_MODEL_PROVIDER_TYPE']));
  const baseUrl = trimmed(env['LMCODE_MODEL_BASE_URL']) ?? DEFAULT_BASE_URL[type];
  ...
  const maxOutputRaw = trimmed(env['LMCODE_MODEL_MAX_OUTPUT_SIZE']);
  const maxOutputSize =
    maxOutputRaw !== undefined
      ? parsePositiveInt(maxOutputRaw, 'LMCODE_MODEL_MAX_OUTPUT_SIZE')
      : undefined;
  const capabilities = parseCapabilities(env['LMCODE_MODEL_CAPABILITIES']) ?? DEFAULT_CAPABILITIES;
  const displayName = trimmed(env['LMCODE_MODEL_DISPLAY_NAME']);
  const reasoningKey = trimmed(env['LMCODE_MODEL_REASONING_KEY']);
  const adaptiveThinking = parseBooleanVar(
    env['LMCODE_MODEL_ADAPTIVE_THINKING'],
    'LMCODE_MODEL_ADAPTIVE_THINKING',
  );
  ...
  const thinkingMode = trimmed(env['LMCODE_MODEL_THINKING_MODE']);
  const thinkingEffort = trimmed(env['LMCODE_MODEL_THINKING_EFFORT']);
  ...
  const defaultThinking = parseBooleanVar(
    env['LMCODE_MODEL_DEFAULT_THINKING'],
  ...
  // LmcodeError(CONFIG_INVALID) on violation, matching the explicit checks above.
