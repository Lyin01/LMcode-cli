 ...
 * synthesized from `LMCODE_MODEL_*` environment variables. Use this everywhere a
...
export function parseConfigString(tomlText: string, filePath = 'config.toml'): ScreamConfig {
  ...
  try {
    ...
    throw new LmcodeError(ErrorCodes.CONFIG_INVALID, `Invalid TOML in ${filePath}: ${error instanceof Error ? error.message : String(error)}`, {
      cause: error,
    });
  }
