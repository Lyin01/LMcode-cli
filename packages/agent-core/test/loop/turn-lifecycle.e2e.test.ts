...
describe('runTurn — turn lifecycle', () => {
  ...
  it('throws LmcodeError(loop.max_steps_exceeded) when steps reach maxSteps', async () => {
    const echo = new EchoTool();
    ...
    expect((error as LmcodeError).code).toBe(ErrorCodes.LOOP_MAX_STEPS_EXCEEDED);
    expect((error as LmcodeError).details).toEqual({ maxSteps: 2 });
