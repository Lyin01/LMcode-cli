...
describe('McpConnectionManager', () => {
  ...
  it('reconnect throws a coded LmcodeError when the server name is unknown', async () => {
    const cm = new McpConnectionManager();
    try {
      await expect(cm.reconnect('nope')).rejects.toBeInstanceOf(LmcodeError);
  ...
  it('reconnect rejects disabled servers without connecting them', async () => {
    ...
    try {
      ...
      await expect(reconnect).rejects.toBeInstanceOf(LmcodeError);
...
describe('Session MCP startup', () => {
  it('stores default MCP OAuth credentials under the configured Scream home', async () => {
    ...
    try {
      ...
      await expect(
        readFile(
          join(processHome, '.lmcode', 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
