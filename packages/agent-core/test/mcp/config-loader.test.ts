...
describe('resolveMcpJsonPaths', () => {
  it('returns the canonical user and project paths', () => {
    const paths = resolveMcpJsonPaths({ cwd: '/work/proj', homeDir: '/home/user/.lmcode' });
    expect(paths.user).toBe('/home/user/.lmcode/mcp.json');
    expect(paths.project).toBe('/work/proj/.lmcode/mcp.json');
  });
});
...
describe('loadMcpServers', () => {
  ...
  it('merges project-local mcp.json with user-global, project overriding on conflict', async () => {
    ...
    await writeJson(join(cwd, '.lmcode', 'mcp.json'), {
      mcpServers: {
  ...
  it('throws LmcodeError(config.invalid) on invalid JSON', async () => {
    ...
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toBeInstanceOf(LmcodeError);
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toMatchObject({
      code: ErrorCodes.CONFIG_INVALID,
    });
  });

  it('throws LmcodeError(config.invalid) on schema violation (unknown transport)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('throws LmcodeError(config.invalid) on schema violation (missing required field)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('honors LMCODE_HOME env var when homeDir is not supplied', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
