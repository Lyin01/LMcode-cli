...
function expectConfigInvalid(fn: () => unknown): void {
  try {
    ...
    expect((error as LmcodeError).code).toBe(ErrorCodes.CONFIG_INVALID);
    return;
  }
...
describe('buildMcpHttpHeaders', () => {
  ...
  it('passes through configured static headers', () => {
    expect(
      buildMcpHttpHeaders(
        { transport: 'http', url: 'https://x', headers: { 'X-Tenant': 'lmcode' } },
        () => undefined,
      ),
    ).toEqual({ 'X-Tenant': 'lmcode' });
  });
  ...
  it('throws LmcodeError(config.invalid) when a configured bearer token env var is empty or missing', () => {
    expectConfigInvalid(() =>
      buildMcpHttpHeaders(
