...
it('runs an agent turn through builtin tool approval and execution', async () => {
  ...
  // The default permission mode is auto, under which Bash would run without
  // emitting requestApproval. This test exercises the manual approval flow,
  // so switch to manual before configure() — configure() resets the event
  // cursor, keeping this setup out of the asserted snapshots, while the
  // recorded mode change still replays correctly in expectResumeMatches.
