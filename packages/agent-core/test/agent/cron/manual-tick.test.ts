 ...
 * `LMCODE_CRON_MANUAL_TICK=1` env disables the auto-tick interval and,
...
describe('CronManager — P1.8 manual tick + SIGUSR1', () => {
  ...
  describe('LMCODE_CRON_MANUAL_TICK=1', () => {
    it('does not install setInterval; tick() must be called manually', async () => {
      vi.stubEnv('LMCODE_CRON_MANUAL_TICK', '1');
  ...
  describe('without LMCODE_CRON_MANUAL_TICK', () => {
  ...
  describe('SIGUSR1', () => {
    // SIGUSR1 binding is opt-in via LMCODE_CRON_MANUAL_TICK=1 so that
    ...
    it('logs swallowed tick() throws to stderr when LMCODE_CRON_DEBUG=1', async () => {
      if (process.platform === 'win32') return;
      vi.stubEnv('LMCODE_CRON_DEBUG', '1');
    ...
    it('does not write to stderr on tick() throw when LMCODE_CRON_DEBUG is unset', async () => {
      if (process.platform === 'win32') return;
      // LMCODE_CRON_DEBUG intentionally NOT set in this test.
    ...
    it('stop() removes the SIGUSR1 listener (no leak)', async () => {
      ...
      // Constructor auto-starts, which binds SIGUSR1 under LMCODE_CRON_MANUAL_TICK=1.
      const manager = new CronManager(stub.agent, { pollIntervalMs: null });
    ...
    it('does not bind when LMCODE_CRON_MANUAL_TICK is unset', async () => {
      if (process.platform === 'win32') return;
