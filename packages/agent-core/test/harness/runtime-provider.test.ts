...
function resolveRuntimeProvider(input: {
  ...
  const manager = new ProviderManager({
    config: input.config,
    lmcodeRequestHeaders: input.lmcodeRequestHeaders,
...
describe('resolveRuntimeProvider model metadata', () => {
  it('uses config model metadata as the source of truth', () => {
    ...
    expect(resolved.provider.model).toBe('lmcode-for-coding');
  });
...
describe('resolveRuntimeProvider LMcode request headers', () => {
  ...
  it('uses only customHeaders when lmcodeRequestHeaders are missing', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('lets provider customHeaders override lmcodeRequestHeaders', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
