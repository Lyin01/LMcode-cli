...
function expectLmcodeErrorCode(fn: () => unknown, code: string): void {
  try {
    fn();
  } catch (error) {
    expect(error).toBeInstanceOf(LmcodeError);
    expect((error as LmcodeError).code).toBe(code);
    return;
  }
...
utility_model = "lmcode/lmcode-for-coding"
...
describe('harness config TOML loader', () => {
  it('parses the current config.toml shape through explicit field mappings', () => {
    ...
    expect(config.defaultModel).toBe('lmcode/lmcode-for-coding');
    expect(config.utilityModel).toBe('lmcode/lmcode-for-coding');
    ...
    expect(config.models?.['lmcode/lmcode-for-coding']).toMatchObject({
      provider: 'managed:lmcode',
      model: 'lmcode-for-coding',
  ...
  it('loads defaults for absent files and writes typed fields without dropping raw sections', async () => {
    ...
    expect(text).toContain('default_model = "lmcode/lmcode-for-coding"');
  ...
  it('creates a parseable default config scaffold without changing runtime defaults', async () => {
    ...
    const text = await readFile(configPath, 'utf-8');
    expect(text).toContain('Runtime settings for LMcode.');
  ...
  it('rejects invalid TOML and invalid schema with LmcodeError(config.invalid)', () => {
    expectLmcodeErrorCode(
...
describe('harness config schema and patch merge', () => {
  ...
  it('deep-merges validated patches while preserving existing typed and raw data', () => {
    ...
    expect(merged.models?.['lmcode/lmcode-for-coding']).toMatchObject({
      provider: 'managed:lmcode',
      model: 'lmcode-for-coding',
      maxContextSize: 262144,
      capabilities: ['tool_use'],
