...
describe('PluginManager', () => {
  it('install() adds a plugin and load() rehydrates it from disk', async () => {
    ...
    manager = new PluginManager({ lmcodeHomeDir: home });
    await manager.load();
    expect(manager.list()).toHaveLength(1);
  ...
  it('install() accepts a .lmcode-plugin manifest', async () => {
    ...
    expect(record.manifestKind).toBe('lmcode-plugin-dir');
  ...
  it.skipIf(process.platform === 'win32')('install() copies a symlinked plugin root into the managed plugins dir', async () => {
    ...
    const reloaded = new PluginManager({ lmcodeHomeDir: home });
    await reloaded.load();
  ...
  it('reload() picks up edits to the managed plugin copy', async () => {
    ...
    await writeFile(
      path.join(managedRoot, 'lmcode.plugin.json'),
  ...
  it('keeps a plugin in error state instead of losing it on a broken manifest', async () => {
    ...
    await manager.install(root);
    await writeFile(
      path.join(await managedPluginRoot(home, 'demo'), 'lmcode.plugin.json'),
      '{ not json',
      'utf8',
    );
  ...
  it('install() from zip-url overwrites existing zip-url plugin', async () => {
    ...
    const zipBuffer2 = await createZipBuffer([
      { name: 'plugin/lmcode.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '2.0.0' }) },
    ]);
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    const cdnZip = await createZipBuffer([
      { name: 'pkg/lmcode.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.0.0' }) },
    ]);
    ...
    const ghZip = await createZipBuffer([
      { name: 'pkg/lmcode.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.1.0' }) },
    ]);
    using _ = mockGithubFetch({
      releaseTag: 'v5.1.0',
      tarball: ghZip,
