...
describe('extractZip', () => {
  ...
  it('prefers a plugin manifest at the archive root', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'lmcode.plugin.json', data: '{"name":"root"}' },
      { name: 'examples/demo/lmcode.plugin.json', data: '{"name":"demo"}' },
    ]);
  ...
  it('detects plugin root with lmcode.plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/lmcode.plugin.json', data: '{"name":"test"}' },
      { name: 'my-plugin/readme.md', data: '# Test' },
    ]);

    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, 'lmcode.plugin.json'), 'utf8');
  ...
  it('detects a single wrapper directory before nested manifests', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'outer/lmcode.plugin.json', data: '{"name":"outer"}' },
      { name: 'outer/inner/lmcode.plugin.json', data: '{"name":"inner"}' },
    ]);
